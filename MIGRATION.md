# Migration — from the old `personal_ai_workflow` to PAW Method

This repo was redesigned from a project-coupled workflow (Rails/Angular + Hitax +
personal evaluation data, all in one repo) into PAW Method: a public, stack-agnostic
core (`~/.paw/core/`) plus user-owned adapters and historic data
(`~/.paw/user/`).
...
personal out of this repo into your local `~/.paw/user/`.
...
and Angular/hitax-ng conventions and skills (`hitax-*`, `paw-add-*`, etc.) tied to a
specific employer's stack and internal libraries (`hitax_ai`, `hitax_hub`, `hitax-ng`,
`HtxApiService`, ...).

To migrate:

1. `paw new-adapter hitax`
2. Move `standards/code_standards.md`, `personal_code_standards.md`,
   ...
   `~/.paw/user/adapters/hitax/standards/` — split/merge into `floor.md` (precedence 1)
...
3. Move `adapters/backend/workflow/3_stdd_instructions.md` and
   ...
   `~/.paw/user/adapters/hitax/workflow/3_implement.md` — pick (or merge) whichever
   applies based on the stack the adapter targets, following the structure of
   `core/workflow/3_implement.md`.
4. Move all `hitax-*` skills and the `paw-*` frontend/backend skills into
   `~/.paw/user/adapters/hitax/skills/`.
...
### 2. Personal evaluation data → `~/.paw/user/historic/`

The old `core/evaluation/monthly/*.md` files (2025-11 through 2026-06) contain real
performance-review content. Move them as-is to `~/.paw/user/historic/`:

```bash
mkdir -p ~/.paw/user/historic
mv core/evaluation/monthly/*.md ~/.paw/user/historic/
paw historic on
```
...
### 3. Active project contexts/specs → `~/.paw/user/projects/<project-slug>/`
...
2. `mkdir -p ~/.paw/user/projects/<project-slug>/{contexts,specs}`
...
- `paw doctor` should report no issues.
- Open a project that matches the `hitax` adapter's `stacks:` and confirm the
  orchestrator loads it.
- Confirm `paw historic on` picks up the current month correctly and that closing a
  task appends to it.
- This repo (PAW Method core) should now contain **no personal data and nothing
